...
void DetectFileChanges(napi_env env, napi_value jsCallbackRef, const std::vector<std::filesystem::path>& oldFiles, const std::vector<std::filesystem::path>& newFiles) {
...
// Callback para iniciar la observación
napi_value StartWatching(napi_env env, napi_callback_info info) {
    ...
    // PrintToConsole(folderPath.c_str());
